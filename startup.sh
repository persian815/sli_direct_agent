...
if ! $PYTHON_PATH -m pip --version &> /dev/null; then
  log "Installing pip..."
  $PYTHON_PATH -m ensurepip --upgrade || handle_error "Failed to ensure pip is installed"
  $PYTHON_PATH -m pip install --upgrade pip || handle_error "Failed to upgrade pip"
fi
...
if [ -f "requirements.txt" ]; then
  ...
  $PYTHON_PATH -m pip install -r requirements.txt || handle_error "Failed to install requirements"
else
  log "WARNING: requirements.txt not found. Installing basic packages..."
  $PYTHON_PATH -m pip install streamlit pandas numpy || handle_error "Failed to install basic packages"
fi
...
$PYTHON_PATH -m streamlit run "$ENTRY_POINT" --server.port 8000 --server.enableCORS false --server.address 0.0.0.0 || handle_error "Failed to start Streamlit"

# If Streamlit fails, try running the Python script directly
if [ $? -ne 0 ]; then
  log "Streamlit failed, trying to run the Python script directly..."
  $PYTHON_PATH "$ENTRY_POINT" || handle_error "Failed to run the Python script directly"
fi
